...
void keyboardChar (GLFWwindow* window, unsigned int key)
{
	switch (key) {
        ...
        case 'd':
        case 'D':
        if(xpos_player<9&&is_obstacle[xpos_player+1][ypos_player]==0&&curr_z[xpos_player+1][ypos_player]-zpos_player<0.5)
        {
        	if(inp_board[xpos_player+1][ypos_player]!='L')restart();
            xpos_player+=1;
            cout<<curr_z[xpos_player+1][ypos_player]<<" "<<zpos_player<<endl;
            if(curr_z[xpos_player+1][ypos_player]-zpos_player<-0.5)health-=20;
            sound2.play();
        }
        else
        {
        ...
        case 'a':
        case 'A':
            if(xpos_player>0&&is_obstacle[xpos_player-1][ypos_player]==0&&curr_z[xpos_player-1][ypos_player]-zpos_player<0.5)
            {
            if(inp_board[xpos_player-1][ypos_player]!='L')restart();
             xpos_player-=1;
             //cout<<curr_z[xpos_player-1][ypos_player]<<" "<<zpos_player<<endl;
        ...
        case 's':
        case 'S':
            if(ypos_player>0&&is_obstacle[xpos_player][ypos_player-1]==0&&curr_z[xpos_player][ypos_player-1]-zpos_player<0.5)
            {
            	if(inp_board[xpos_player][ypos_player-1]!='L')restart();
            ypos_player-=1;
            //cout<<curr_z[xpos_player][ypos_player-1]<<" "<<zpos_player<<endl;
            if(curr_z[xpos_player][ypos_player-1]-zpos_player<-0.5)health-=20;
        	sound2.play();
        }
        else
        {
        ...
        case 'w':
        case 'W':
        	if(ypos_player<9&&is_obstacle[xpos_player][ypos_player+1]==0&&curr_z[xpos_player][ypos_player+1]-zpos_player<0.5)
        	{
            	ypos_player+=1;
            	if(inp_board[xpos_player][ypos_player+1]!='L')restart();
            	//cout<<curr_z[xpos_player][ypos_player+1]<<" "<<zpos_player<<endl;
...
void cameraposition(){
	if(cstate==1){viewRadius=10;}
    if(cstate==0){viewRadius=3;}
    if(campos==0)
    {
      // top view
        act_eye[0]= eyefrom[1]-0.3 ;
        act_eye[1]= eyefrom[2]-1.2;
        act_eye[2] = eyefrom[3]+5.2;
        target[0]= targetto[1];
        target[1]= targetto[2];
        target[2] = targetto[3];
         camup[0]=0;
    ...
    else if(campos==1)
    {
      // Adventure view
        ...
        act_eye[0]=xpos_player-7.1;
        act_eye[1]=ypos_player-2.6;
        act_eye[2]=zpos_player+2.5f;
        target[0]=xpos_player;
        target[1]=ypos_player;
        target[2]=zpos_player;
        camup[0]=0;
        camup[1]=0;
        camup[2]=1;
        //cout<<act_eye[0]<<" "<<act_eye[1]<<" "<<act_eye[2]<<" "<<target[0]<<" "<<target[1]<<" "<<target[2]<<endl; 

    }
    else if(campos==2)
    {
      // tower view
        act_eye[0]= eyefrom[1]+-0.3-0.6-0.4+panx ;
        act_eye[1]= eyefrom[2]+-1.2-5.3+pany;
        act_eye[2] = eyefrom[3]+5.2-3.1+panz ;
        target[0]= targetto[1];
        target[1]= targetto[2];
        target[2] = targetto[3];
        camup[0]=0;
...
void draw (GLFWwindow* window)
{
	...
	static float c = 0;
	//c++;
   glm::vec3 eye (act_eye[0]+panx, act_eye[1]+pany, act_eye[2]+panz);	// Target - Where is the camera looking at.  Don't change unless you are sure!!
	glm::vec3 target (target[0],target[1]+mouposx,target[2]+mouposy);
	// Up - Up vector defines tilt of camera.  Don't change unless you are sure!!
	glm::vec3 up (camup[0], camup[1], camup[2]);
	...
	// Matrices.view = glm::lookAt(glm::vec3(act_eye[1],act_eye[2],act_eye[3]), glm::vec3(targetto[1]+0*mouposx,targetto[2],targetto[3]+0*mouposy), glm::vec3(camup[0], camup[1], camup[2])); // Fixed camera for 2D (ortho) in XY plane
	...
	if(our_count>10000000000000000)our_count=0;
	//map
	for(int i=0;i<10;i++)
	{
  		for(int j=0;j<10;j++)
  		{
  			if((i>xpos_player+viewRadius || i<xpos_player-viewRadius) || (j>ypos_player+viewRadius || j<ypos_player-viewRadius))
  			{
  				continue;
  			}
  			//cout<<i<<" "<<j<<endl;
	...
	//cout<<zpos_player<<" "<<curr_z[xpos_player+1][ypos_player]<<endl;
	...
	for(int i=0;i<10;i++)
	{
  		for(int j=0;j<10;j++)
  		{
  			...
  			{
  				if(xpos_player==0 && ypos_player==0)win(window);
  				Matrices.model = glm::mat4(1.0f);
	...
	// Transform the text
	Matrices.model = glm::mat4(1.0f);
	glm::mat4 translateText = glm::translate(glm::vec3(-6,+6,-2));
	...
	// Render font
	if(lives==3 && health == 100)
	GL3Font.font->Render("Level: 1 \n Lives: 3 \n Health: 100");
	if(lives==3 && health == 80)
	GL3Font.font->Render("Level: 1 \n Lives: 3 \n Health: 80");
	if(lives==3 && health == 60)
	GL3Font.font->Render("Level: 1 \n Lives: 3 \n Health: 60");
	if(lives==3 && health == 40)
	GL3Font.font->Render("Level: 1 \n Lives: 3 \n Health: 40");
	if(lives==3 && health == 20)
	GL3Font.font->Render("Level: 1 \n Lives: 3 \n Health: 20");
	if(lives==2 && health == 100)
	GL3Font.font->Render("Level: 1 \n Lives: 2 \n Health: 100");
	if(lives==2 && health == 80)
	GL3Font.font->Render("Level: 1 \n Lives: 2 \n Health: 80");
	if(lives==2 && health == 60)
	GL3Font.font->Render("Level: 1 \n Lives: 2 \n Health: 60");
	if(lives==2 && health == 40)
	GL3Font.font->Render("Level: 1 \n Lives: 2 \n Health: 40");
	if(lives==2 && health == 20)
	GL3Font.font->Render("Level: 1 \n Lives: 2 \n Health: 20");
	if(lives==1 && health == 100)
	GL3Font.font->Render("Level: 1 \n Lives: 1 \n Health: 100");
	if(lives==1 && health == 80)
	GL3Font.font->Render("Level: 1 \n Lives: 1 \n Health: 80");
	if(lives==1 && health == 60)
	GL3Font.font->Render("Level: 1 \n Lives: 1 \n Health: 60");
	if(lives==1 && health == 40)
	GL3Font.font->Render("Level: 1 \n Lives: 1 \n Health: 40");
	if(lives==1 && health == 20)
	GL3Font.font->Render("Level: 1 \n Lives: 1 \n Health: 20");
	if(health<20){restart();}
	if(lives<1)lose(window);
...
int main (int argc, char** argv)
{
	...
	if (!buffer1.loadFromFile("level.wav"))
	        return -1;	
	sound1.setBuffer(buffer1);
	if (!buffer2.loadFromFile("accept.wav"))
	        return -1;
	sound2.setBuffer(buffer2);
	if (!buffer3.loadFromFile("reject.wav"))
	        return -1;	
	sound3.setBuffer(buffer3);
	if (!buffer4.loadFromFile("over.wav"))
	        return -1;	
